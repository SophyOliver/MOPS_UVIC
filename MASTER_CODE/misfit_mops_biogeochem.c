...
/* If runscript specifies L operator to be used to convert to a common grid before calculating the misfit, then create a template vector for the common grid */
    ...
    ierr = PetscOptionsHasName(PETSC_NULL,"-convert_misfit_grid",&doLoperator);CHKERRQ(ierr);

    if (doLoperator) {
  
      ierr = PetscOptionsGetString(PETSC_NULL,"-L_operator_file",L_file,PETSC_MAX_PATH_LEN-1,&flg);CHKERRQ(ierr);
      if (!flg) SETERRQ(PETSC_COMM_WORLD,1,"Must provide petsc file name of the L operator");
  
      ierr = PetscOptionsGetInt(PETSC_NULL,"-common_grid_vector_length",&common_grid_vec_len,&flg);CHKERRQ(ierr);
